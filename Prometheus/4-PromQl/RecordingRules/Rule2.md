...
در این‌جا ترجمهٔ صفحهٔ «روندها/قواعد ضبط» در مستندات Prometheus را برایتان آورده‌ام:

---

### قواعد ضبط

یک اسکِم نام‌گذاری ثابت برای قواعد ضبط باعث می‌شود که معنیِ یک قاعده به‌راحتی در نگاه اول قابل فهم باشد. همچنین از بروز اشتباهات، مثلاً محاسبات نادرست یا بی‌معنی جلوگیری می‌کند.
در این صفحه، نام‌گذاری صحیح و گردآوری (Aggregation) برای قواعد ضبط توضیح داده شده است.
...
* قواعد ضبط باید به شکل کلی زیر باشند:
...
Prometheus از دو نوع قاعده پشتیبانی می‌کند که می‌توان آن‌ها را پیکربندی کرده و در فواصل منظم ارزیابی نمود: **قواعد ضبط** (recording rules) و **قواعد هشداردهی** (alerting rules). 
...
* `limit: <int>` | پیش‌فرض = 0 — محدودیت تعداد سر‌ی‌های تولیدشده توسط قواعد ضبط یا هشداردهی
