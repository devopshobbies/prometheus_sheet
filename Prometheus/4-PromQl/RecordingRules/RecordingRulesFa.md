...
در Prometheus، **Recording Rules** یکی از ویژگی‌های کلیدی برای **بهینه‌سازی کوئری‌ها و ذخیره مقادیر محاسبه‌شده** است. 
...
‏recording rules در فایل‌هایی جدا از `prometheus.yml` شما قرار می‌گیرند که به عنوان «فایل‌های قوانین» (rule files) شناخته می‌شوند. همانند `prometheus.yml`، فایل‌های قوانین نیز از فرمت YAML استفاده می‌کنند. شما می‌توانید محل قرارگیری فایل‌های قوانین خود را با استفاده از فیلد سطح بالای `rule_files` در `prometheus.yml` خود مشخص کنید. به عنوان مثال، مثال ۱ یک فایل قانون به نام `rules.yml` را بارگذاری می‌کند، علاوه بر اینکه دو هدف (target) را نیز جمع‌آوری (scrape) می‌کند.

مثال ۱. `prometheus.yml` که دو هدف را جمع‌آوری و یک فایل قانون را بارگذاری می‌کند.
```yaml
global:
  scrape_interval: 10s
...
**کاردینالیتی در Prometheus به تعداد مجموعه‌های سری‌های زمانی (Time Series) منحصر به فرد در پایگاه داده آن اشاره دارد.**

هر چه کاردینالیتی بالاتر باشد، Prometheus باید سری‌های زمانی بیشتری را ذخیره، ایندکس و پردازش کند.

---

##### تشریح اجزای تشکیل‌دهنده کاردینالیتی

برای درک کامل، باید ببینیم یک سری زمانی چگونه ساخته می‌شود. هر سری زمانی با ترکیب **نام متریک (Metric Name)** و **برچسب‌های (Labels)** آن به صورت یکتا شناسایی می‌شود.

**مثال:**

فرض کنید متریکی به نام `http_requests_total` داریم که با برچسب‌های `method` (متد HTTP) و `endpoint` (آدرس درخواست) همراه است.
...
در این مثال، ما ۴ سری زمانی متفاوت داریم. حالا تصور کنید:

*   اگر `endpoint`های شما زیاد باشد (مثلاً شامل `id` کاربر شود: `/api/users/123`, `/api/users/456` و...).
*   یا اگر برچسب دیگری مثل `user_id` را مستقیماً به متریک اضافه کنید.

در این صورت، تعداد ترکیب‌های ممکن به سرعت افزایش یافته و **کاردینالیتی منفجر می‌شود**.

---

##### چرا کاردینالیتی مهم است؟ (اثرات کاردینالیتی بالا)

کاردینالیتی بالا مستقیماً بر عملکرد و سلامت Prometheus تاثیر می‌گذارد:

1.  **مصرف حافظه (RAM) بسیار بالا:** Prometheus برای هر سری زمانی، یک ایندکس در حافظه ایجاد می‌کند. کاردینالیتی بالا باعث می‌شود حافظه RAM به سرعت پر شده و Prometheus با خطای `Out Of Memory (OOM)` کرش کند.
2.  **افت عملکرد (Performance):** کوئری‌ها (مخصوصاً آنهایی که از توابعی مثل `regex` روی برچسب‌ها استفاده می‌کنند) روی داده‌های با کاردینالیتی بالا بسیار کند اجرا می‌شوند.
3.  **مصرف دیسک بالا:** هر سری زمانی داده ذخیره می‌کند، بنابراین کاردینالیتی بالا مستلزم فضای ذخیره‌سازی بیشتر است.

یک کاردینالیتی کنترل‌نشده می‌تواند در عرض چند دقیقه کل سرور Prometheus را از کار بیندازد.

---

##### عوامل رایج ایجاد کاردینالیتی بالا (Anti-patterns)

*   **استفاده از برچسب‌هایی که مقادیر منحصربه‌فرد زیادی دارند:**
    *   `user_id`
    *   `session_id`
    *   `ip_address` (در صورتی که تعداد آیپی‌ها بسیار زیاد باشد)
    *   `request_id` یا `trace_id`
    *   `error_message` (اگر متن خطا کامل و منحصربه‌فرد باشد)
*   **ایجاد متریک‌های جدید در داخل برنامه:** مثلاً نام متریک را با یک `id` تغییر دهید.
*   **افزودن برچسب از نوع تاریخ/زمان:** مثلاً `date="2023-10-25"`. این برچسب هر روز جدید می‌شود و سری‌های زمانی قدیمی هرگز استفاده نمی‌شوند اما در حافظه باقی می‌مانند.

---

##### چگونه کاردینالیتی را مدیریت و کنترل کنیم؟

1.  **طراحی صحیح متریک‌ها و برچسب‌ها (مهمترین راهکار):**
    *   از برچسب‌ها برای ابعادی (dimensions) استفاده کنید که مقادیر محدود و قابل شمارشی دارند (مانند `method=GET|POST|PUT|DELETE`، `status=200|404|500`، `endpoint=/api/users|/api/products`).
    *   از استفاده از برچسب برای مقادیر با کاردینالیتی بالا (مانند `user_id`) به شدت **خودداری کنید**. به جای آن، از توابع aggregate مانند `sum()`, `avg()`, `count()` استفاده کنید. مثلاً به جای ردیابی درخواست برای هر کاربر، تعداد کل درخواست‌ها یا میانگین لیتنسی را محاسبه کنید.

2.  **استفاده از `rate()` و `increase()`:** هنگام کار با متریک‌های Counter (مانند تعداد درخواست‌ها) تقریباً همیشه باید از تابع `rate()` یا `increase()` در کوئری‌ها استفاده کنید. این توابع خود به مدیریت کاردینالیتی کمک می‌کنند.

3.  **فیلتر کردن متریک‌ها در سمت ارسال (Scrape):** با استفاده از `relabel_configs` و قانون `drop` می‌توانید متریک‌ها یا برچسب‌های خاصی را که کاردینالیتی ایجاد می‌کنند، حذف کنید.

4.  **نظارت بر کاردینالیتی خود Prometheus:** می‌توانید از کوئری زیر برای دیدن ۱۰ متریکی که بیشترین کاردینالیتی را ایجاد می‌کنند استفاده کنید:
    ```prometheus
    topk(10, count by (__name__)({__name__=~".+"}))
    ```
    همچنین متریک `prometheus_tsdb_head_series` تعداد کل سری‌های زمانی را نشان می‌دهد.

### خلاصه

| مفهوم | توضیح |
| :--- | :--- |
| **کاردینالیتی پایین** | تعداد کمی سری زمانی منحصربه‌فرد. عملکرد عالی و مصرف منابع کم. |
| **کاردینالیتی بالا** | تعداد بسیار زیادی سری زمانی منحصربه‌فرد. منجر به مصرف RAM شدید، کندی و کرش می‌شود. |
| **علت اصلی** | استفاده از برچسب‌هایی با مقادیر منحصربه‌فرد زیاد (مانند `id`ها). |
| **راهکار اصلی** | طراحی هوشمندانه متریک‌ها و استفاده از برچسب‌هایی با مقادیر محدود و قابل شمارش. |

در نتیجه، مدیریت کاردینالیتی یک **مهارت حیاتی** برای هر ادمین Prometheus است و موفقیت شما در مانیتورینگ به درک و کنترل آن بستگی دارد.
...
نمی‌توانید از توابع Range Vector بر روی خروجی توابعی که وکتورهای لحظه‌ای (instant vectors) تولید می‌کنند، استفاده کنید. به عنوان مثال،
 `max_over_time(sum without(instance)(rate(x_total[5m]))[1h])` 
 امکان‌پذیر نیست و یک خطای تجزیه (parse error) ایجاد می‌کند. در حالی که PromQL دارای «زیرکوئری‌ها» (subqueries) است، می‌توانید از recording rules برای رسیدن به همین نتیجه استفاده کنید:
