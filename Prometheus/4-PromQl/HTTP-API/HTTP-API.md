...
*‏   <rfc3339 | unix_timestamp>: مهرهای زمانی ورودی می‌توانند به فرمت RFC3339 یا به عنوان مهر زمانی یونیکس بر حسب ثانیه ارائه شوند، با امکان دقت زیرثانیه‌ای (با اعشار). مهرهای زمانی خروجی همیشه به عنوان مهرهای زمانی یونیکس بر حسب ثانیه نمایش داده می‌شوند.
...
توجه: نام پارامترهای پرس‌وجو که ممکن است تکرار شوند، با `[]` به پایان می‌رسند.

پرس‌وجوهای عبارتی (Expression queries)

عبارات زبان پرس‌وجو را می‌توان در یک لحظه مشخص یا در یک بازه زمانی ارزیابی کرد. بخش‌های زیر، نقاط پایانی API را برای هر نوع پرس‌وجوی عبارتی توضیح می‌دهند.

پرس‌وجوهای لحظه‌ای (Instant queries)

نقطه پایانی زیر، یک پرس‌وجوی لحظه‌ای را در یک نقطه زمانی خاص ارزیابی می‌کند:

`GET /api/v1/query`
`POST /api/v1/query`

پارامترهای پرس‌وجوی URL:

‏ *  <query=<string: رشته پرس‌وجوی عبارت پرومتئوس.
...
*  ‏ lookback_delta=number: دوره بازنگری (lookback period) را فقط برای این پرس‌وجو بازنویسی می‌کند. اختیاری.

اگر پارامتر `time` حذف شود، زمان فعلی سرور استفاده می‌شود.

می‌توانید این پارامترها را مستقیماً در بدنه درخواست با استفاده از متد POST و هدر `Content-Type: application/x-www-form-urlencoded` کدگذاری URL کنید. این کار زمانی مفید است که یک پرس‌وجوی بزرگ را مشخص می‌کنید که ممکن است از محدودیت کاراکتر URL سمت سرور فراتر رود.

بخش `data` نتیجه پرس‌وجو دارای قالب زیر است:
...
‏`<value>` به داده‌های نتیجه پرس‌وجو اشاره دارد که بسته به `resultType` قالب‌های متفاوتی دارد. به فرمت‌های نتیجه پرس‌وجوی عبارت مراجعه کنید.
...
پرس‌وجوهای بازه‌ای (Range queries)

نقطه پایانی زیر یک پرس‌وجوی عبارتی را در یک بازه زمانی ارزیابی می‌کند:
...
* ‏  step=duration | float: عرض گام (step width) وضوح پرس‌وجو در قالب مدت زمان یا عدد اعشاری بر حسب ثانیه.
*‏   timeout=duration: مهلت ارزیابی. اختیاری. به طور پیش‌فرض، مقدار پرچم `-query.timeout` است و توسط آن محدود می‌شود.
...
مثال زیر عبارت `up` را در یک بازه 30 ثانیه‌ای با وضوح پرس‌وجوی 15 ثانیه ارزیابی می‌کند.
...
قالب‌بندی عبارات پرس‌وجو (Formatting query expressions)
...
بخش `data` نتیجه پرس‌وجو یک رشته حاوی عبارت پرس‌وجوی قالب‌بندی شده است. توجه داشته باشید که هرگونه نظر در رشته قالب‌بندی شده حذف می‌شود.
...
بخش `data` نتیجه پرس‌وجو یک رشته حاوی AST عبارت پرس‌وجوی تجزیه شده است.
...
پرس‌وجوی فراداده (Querying metadata)

پرومتئوس مجموعه‌ای از نقاط پایانی API را برای پرس‌وجوی فراداده در مورد سری‌ها و برچسب‌های آن‌ها ارائه می‌دهد.
...
بخش `data` نتیجه پرس‌وجو شامل لیستی از اشیا است که شامل جفت‌های نام/مقدار برچسب هستند که هر سری را شناسایی می‌کنند. توجه داشته باشید که زمان‌های شروع و پایان تقریبی هستند و نتیجه ممکن است شامل مقادیر برچسب برای سری‌هایی باشد که در بازه زمانی مشخص شده هیچ نمونه‌ای ندارند.
...
پرس‌وجوی مقادیر برچسب (Querying label values)
...
این مثال تمام مقادیر برچسب را برای برچسب `http_status_code` پرس‌وجو می‌کند:
...
این مثال تمام مقادیر برچسب را برای برچسب `http.status_code` پرس‌وجو می‌کند:
...
پرس‌وجوی نمونه‌ها (Querying exemplars)

این بخش آزمایشی است و ممکن است در آینده تغییر کند. نقطه پایانی زیر لیستی از نمونه‌ها را برای یک پرس‌وجوی PromQL معتبر در یک بازه زمانی خاص بازمی‌گرداند:
...
قالب‌های نتیجه پرس‌وجوی عبارت (Expression query result formats)

پرس‌وجوهای عبارتی ممکن است مقادیر پاسخ زیر را در ویژگی `result` بخش `data` بازگردانند. نگه‌دارنده‌های `<sample_value>` مقادیر نمونه عددی هستند. JSON از مقادیر اعشاری خاص مانند NaN، Inf و -Inf پشتیبانی نمی‌کند، بنابراین مقادیر نمونه به عنوان رشته‌های JSON نقل‌قول شده منتقل می‌شوند تا اعداد خام.
...
پارامتر پرس‌وجوی `state` به تماس‌گیرنده اجازه می‌دهد تا بر اساس هدف‌های فعال یا حذف شده فیلتر کند (به عنوان مثال، `state=active`، `state=dropped`، `state=any`). توجه داشته باشید که یک آرایه خالی همچنان برای هدف‌هایی که فیلتر شده‌اند، بازگردانده می‌شود. سایر مقادیر نادیده گرفته می‌شوند.
...
پارامتر پرس‌وجوی `scrapePool` به تماس‌گیرنده اجازه می‌دهد تا بر اساس نام مجموعه جمع‌آوری (scrape pool) فیلتر کند.
...
پرس‌وجوی فراداده هدف (Querying target metadata)
...
بخش `data` نتیجه پرس‌وجو شامل لیستی از اشیا است که حاوی فراداده معیار و مجموعه برچسب هدف هستند.
...
پرس‌وجوی فراداده معیار (Querying metric metadata)
...
بخش `data` نتیجه پرس‌وجو شامل یک شیء است که هر کلید آن یک نام معیار و هر مقدار آن لیستی از اشیاء فراداده منحصر به فرد است، همانطور که برای آن نام معیار در تمام هدف‌ها نمایش داده می‌شود.
...
بخش `data` نتیجه پرس‌وجو شامل موارد زیر است:
...
‏Snapshot یک عکس فوری از تمام داده‌های فعلی را در `snapshots/<datetime>-<rand>` در دایرکتوری داده TSDB ایجاد می‌کند و آن دایرکتوری را به عنوان پاسخ بازمی‌گرداند. این عملیات به صورت اختیاری از ایجاد عکس فوری از داده‌هایی که فقط در بلوک سر (head block) وجود دارند و هنوز به دیسک فشرده نشده‌اند، صرف نظر می‌کند.
...
نکته: این نقطه پایانی نمونه‌ها را از سری‌ها به عنوان حذف شده علامت‌گذاری می‌کند، اما لزوماً از بازگرداندن فراداده‌های سری مرتبط در پرس‌وجوهای فراداده برای محدوده زمانی مربوطه (حتی پس از پاک کردن سنگ‌قبرها) جلوگیری نمی‌کند. میزان دقیق حذف فراداده یک جزئیات پیاده‌سازی است که ممکن است در آینده تغییر کند.
جدید در نسخه 2.1 و پشتیبانی از PUT از نسخه 2.9
