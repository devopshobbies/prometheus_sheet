...
### 2. کمترین مصرف حافظه توسط پادها
...
### 3. بیشترین خطای HTTP در سرویس‌ها
...
### 4. پادهایی با کمترین ورودی شبکه
...
1. ‏`topk` و `bottomk` فقط روی **مقادیر instance vector** کار می‌کنن، نه روی range vector (مثل `[5m]`). پس اول باید توابعی مثل `rate()`، `avg_over_time()`، یا `sum()` استفاده بشن تا مقدار فعلی از داده‌های تاریخی ساخته بشه.

2. ‏`k` باید **عدد ثابت و صحیح** باشه، مثل `5` یا `10`.

3. اگه سری‌های زمانی هم‌مقدار باشن، ممکنه Prometheus انتخاب‌های مختلفی برگردونه، چون این تابع‌ها تضمین نمی‌کنن که کدوم سری برگرده در صورت تساوی.
...
# ✅ 2. `quantile_over_time(scalar q, range-vector)`
...
# ✅ 3. `histogram_quantile(q, buckets)`
